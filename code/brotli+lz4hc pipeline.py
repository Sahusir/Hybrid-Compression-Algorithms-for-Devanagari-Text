...
def hybrid_compress(input_file, output_file):
    ...
    brotli_compressed = brotli.compress(data_bytes, quality=6, lgwin=22)
    compressed_data = lz4.frame.compress(brotli_compressed, compression_level=6)
...
def hybrid_decompress(input_file, output_file):
    ...
    lz4_decompressed = lz4.frame.decompress(compressed_data)
    decompressed_data = brotli.decompress(lz4_decompressed).decode()
