...
def hybrid_compress(input_file, output_file):
    ...
    compressed_data = zstd.compress(lz4hc_compressed, level=6)
...
def hybrid_decompress(input_file, output_file):
    ...
    zstd_decompressed = zstd.decompress(compressed_data)
    decompressed_data = lz4.frame.decompress(zstd_decompressed).decode()
