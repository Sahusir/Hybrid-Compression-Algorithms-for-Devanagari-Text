...
def hybrid_compress(input_file, output_file):
    ...
    lz4hc_compressed = lz4.frame.compress(data_bytes, compression_level=6)
    compressed_data = brotli.compress(lz4hc_compressed, quality=6, lgwin=22)
...
def hybrid_decompress(input_file, output_file):
    ...
    brotli_decompressed = brotli.decompress(compressed_data)
    decompressed_data = lz4.frame.decompress(brotli_decompressed).decode()
