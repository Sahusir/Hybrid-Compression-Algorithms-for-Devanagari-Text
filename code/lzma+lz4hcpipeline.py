...
def hybrid_compress(input_file, output_file):
    ...
    lzma_compressed = lzma.compress(data_bytes, preset=6)
    compressed_data = lz4.frame.compress(lzma_compressed, compression_level=6, block_linked=True, block_size=4*1024*1024)
...
def hybrid_decompress(input_file, output_file):
    ...
    lzma_decompressed = lz4.frame.decompress(compressed_data)
    decompressed_data = lzma.decompress(lzma_decompressed).decode()
